...
: A Lavish wake whose source id matches `bin/fm-procevent-lavish.sh source-id "$(bin/fm-projets-board.sh path)"` is a projets page result; load the `projets` skill's page-wake handling, and expect no keyed answer from it because that page is never bound to the keyed-answer intake.
