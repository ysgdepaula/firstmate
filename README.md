...
- **Reuse existing evidence** - before commissioning an investigation, firstmate searches this home's records with [`fm-prior-art.sh`](bin/fm-prior-art.sh); its header and `--help` explain usage, sources, date labels, and search limits.
...
| `/projets`         | Open the per-project Lavish overview, including the brain card, for progress, decisions, pages, costs, events, and meetings, plus “à valider” for pending decisions; see [Projects page](docs/configuration.md#projects-page-configprojetsjson-dataprojets-coutsjson-dataprojets-agendajson) for setup and decision controls, and the [projets skill](.agents/skills/projets/SKILL.md) for page-answer handling |
