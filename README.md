...
| `/projets`         | Build and arm the per-project Lavish page: a rail of the captain's projects, one project at a time, seven blocks per project and three badges, composed from the same bounded fleet state as Bearings plus the private project table; a page button sends a choice back to firstmate, which asks again in chat before acting |
