...
FM_PUSH_LOCK_DIR=                       # directory holding the machine-wide publication lock that serializes pushes and merges across every home on one machine; default $XDG_STATE_HOME/firstmate (bin/fm-push-lock.sh)
FM_PUSH_LOCK_TIMEOUT_SECS=900           # how long a push or merge waits for the one ahead of it before refusing with the holder's pid
...
FM_TEST_JOBS=                           # operator ceiling on bin/fm-test-run.sh's worker count; lowers a resolved or explicit --jobs, never raises it
FM_TEST_QUEUE_TIMEOUT_SECS=3600         # how long a suite run waits for the one already running on this machine before refusing; --no-queue runs beside it instead
FM_TEST_QUEUE_DIR=                      # directory holding bin/fm-test-run.sh's machine-wide run-queue lock; default $XDG_STATE_HOME/firstmate
