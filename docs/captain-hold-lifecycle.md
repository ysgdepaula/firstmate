...
The accepted record is the same one the live check requires, so what changed is only where it is read; an entry with no such record is still refused, by name, with the exact hold and answer commands that make the call durable again.
