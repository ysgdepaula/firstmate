...
Two cases in that family need no beads install and always run: a stubbed tasks-axi that fails any markdown file override proves the captain-hold hold, answer, and close mutations reach a beads-configured home without one; and a stubbed beads home still carrying a pre-migration `data/done-archive.md` proves that stale markdown archive settles nothing on a migrated home, so an entry whose only closed row lives in it is refused by both gates, by name, without the archive being read or named.
