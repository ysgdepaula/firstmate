...
Backlog retention keeps only the configured recent Done rows, so an entry whose call was answered several passes ago names no live task; both gates then accept it on the closed row surviving in the configured Done archive, or on a `resolved` close for its key in the origin's status log, and name on stderr which entry was treated as purged and which record proved it.
