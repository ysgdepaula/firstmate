...
The status log is consulted only when the archive holds no row for the entry under any identity, and an archive path that exists but cannot be opened refuses naming that archive as unreadable rather than claiming it holds no answer.
