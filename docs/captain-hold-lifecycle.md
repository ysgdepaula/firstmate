...
Backlog retention keeps only the configured recent Done rows, so an entry whose call was answered several passes ago names no live task; both gates then establish that the backlog carries no such row and accept it on the closed row surviving in the configured Done archive, or on a `resolved` close for one of its identities in the origin's status log, and name on stderr which entry was treated as purged and which record proved it.
The archive is searched under the entry id and, for a concrete origin, under the legacy derived identity too, the two identities this home's own records can carry the entry under, and the status log is looked up under those same identities because a pre-collapse call can have been closed on the channel under its composed one.
Resolving a live row follows a longer ladder that also reaches the rows a markdown-to-beads migration rehomed, but those are backend row names that no markdown Done archive and no status key can carry, so they belong to that ladder alone.
A purge frees an id for reuse and retention appends a fresh archived section without deduping ids, so the newest archived row under an identity is the one that says whether that call was closed with an answer, an older answered row beneath it proves nothing, and the refusal names how many rows that identity has there.
The two records are ordered rather than independent: a row actually found in the archive is authoritative about itself, so one closed with no resolution record refuses by name under the identity it was found beneath, the status log is never read behind it, and rotating a backlog can never turn a refusal into an acceptance.
The status log is consulted only when the archive holds no row for the entry under any identity, and neither record is concluded from without being opened: a path that exists but cannot be read refuses by name, while a path nothing has ever written to genuinely records nothing.
The archive a home rotates into mirrors tasks-axi's own precedence: the backlog root's `.tasks.toml` `[markdown] archive` when one names it, then that same key in the user config at `$HOME/.tasks-axi/config.toml`, and otherwise tasks-axi's own default beside the backlog file, because a config that names no archive still has one.
Only a home whose resolved backend is markdown has such an archive: on any other backend no markdown Done archive is authoritative, so none is read and none is named, and the status log decides under the same identities, because a markdown-to-beads migration leaves the pre-migration `backlog.md` and `done-archive.md` on disk and an answer archived before that migration says nothing about the call that lives in the migrated graph now.
The accepted record is the same one the live check requires, so what changed is only where it is read; an entry with no such record is still refused, by name, naming every identity and record it searched, with the exact hold and answer repair commands that make the call durable again.
A row that cannot be read is read uncertainty rather than a purge, so it refuses too and surfaces the underlying read failure, exactly as an unreadable ship hold refuses cleanup.
...
Maintainer-verified against tasks-axi 0.2.5: a config carrying `[markdown] path`/`done_keep` with no `archive` key, and a backlog root with no `.tasks.toml` at all, both still rotate closed rows out of the backlog.
Running `tasks-axi add <id> --file data/backlog.md` then `tasks-axi done <id> --keep 0 --file data/backlog.md` from the backlog root wrote the archived rows to `data/done-archive.md`, the sibling of the `--file` backlog, in both configurations.
An unset `archive` key therefore names tasks-axi's default archive, never the absence of one, which is why `fm_backlog_archive_file` resolves that default instead of reporting that the home keeps no archive.
...
It proves: cleanup of a finished task whose own row is the captain call leaves that call open, queued, held, carrying its deliverable, and visible in Bearings' Captain's Call, leaves no pending record behind, survives a `--force` cleanup, and closes only when `answer` records the captain's words, while an ordinary finished task in the same home still closes with its report link; an interrupted cleanup leaves the row In flight and untouched with its pending record, and the next session start retains it as queued and held with the deliverable recorded; a relocated data directory keeps the retention in its one configured backlog; a ship row whose captain hold cannot be read refuses cleanup before any destructive step and surfaces the read failure; the reconstructed silent-divergence case is signalled - a status resolution over a still-open captain-held task reaches both `diverged` and the drain's `RECORD DIVERGENCE` section, under the collapsed and the legacy identity alike, while the backlog task, its hold, and the status log all survive the report unchanged and the printed hint names both reconciliation directions; the false-signal boundary holds - a captain call with no routed work item, a verified `captain-held` transfer, a still-open status decision, an already answered call, and an ordinary task whose keyed question was answered all stay silent; a report-only unresolved captain call refuses `--none` completion before teardown can erase the source; non-forced scout teardown always requires the durable inventory verification; the recorded-answer guard (a bare `tasks-axi done` close fails `verify` until `answer` records the captain's word, and an ordinary finished task cannot be dressed up as an answered call); the purged-entry cases driven through tasks-axi's own Done retention (an answered call archived out of the backlog passes both gates and names the entry and the archived answer that proved it, a pre-collapse call whose attestation records the short key passes on the archived answer of its composed legacy identity, a call the origin's status log records as resolved passes when the home's real archive was read and holds no row for it, an entry with no such record anywhere is refused by name with its hold and answer repair commands and never as an unnamed task, that refusal names the legacy identity it also searched and omits the attestation-edit line on an origin that has no attestation file, an archived row closed with no recorded answer is still refused even when the origin's status log records a resolved close over it, so rotation alone cannot flip the verdict, an archive that exists but cannot be opened refuses naming it unreadable instead of claiming it holds no answer, an origin status log that exists but cannot be opened is named unreadable the same way instead of being reported as recording no close, a home whose config names no archive is still proved through the archived answer tasks-axi rotated into its default archive and still refuses an unanswered rotation there, a captain-call id reused after a purge is judged on its newest archived row alone, so an older archived answer under a later bare close refuses both gates and the refusal says how many rows that identity carries, a pre-collapse call whose close was recorded on the status side under its composed identity passes and names the key that was read, and a still-live call whose row cannot be read refuses both gates with the read failure named and records no attestation); answer-time closure through a bound channel with task-id keys, including the `release` close mode, mode-matched replay idempotence, and the refusal of drifted, mode-mismatched, absent, unheld, and already-closed keys; the chat channel reaching the same intake; hold-set stamping that precedes visible hold state, preserves an active lifecycle's timestamp, and resets after release; interrupted answer closure retaining the stamp until close and restoring resolution-first ordering on retry; deferral through `--until` leaving `captain_actionable` false until due; and every legacy path (composed identities through the shim, pre-collapse `decision_keys=` metadata, routed-resolution replay, and a concrete-origin binding).
The markdown-to-beads migration family runs the same suite's beads fixture (bd-driven scratch graph, self-skipping on markdown-only tasks-axi installs) and proves: `verify` and `complete` resolve an attested legacy id through a migrated row's marker note, through the configured prefix when no row carries a note - naming the resolved row in the completion line - and through the marker note of a pre-collapse derived identity; a marker-noted row wins over an unrelated captain-held row occupying the bare prefix namesake; an unresolvable id is refused once naming the id (never an empty name); a pre-migration `data/done-archive.md` left on disk proves nothing there, so an entry whose only closed row lives in that stale markdown archive is refused by both gates, by name, without the archive being read or named; and the attested id stays in `decision_keys=` for idempotent re-verification.
