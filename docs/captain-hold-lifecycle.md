...
Backlog retention keeps only the configured recent Done rows, so an entry whose call was answered several passes ago names no live task; both gates then establish that the backlog carries no such row and accept it on the closed row surviving in the configured Done archive, or on a `resolved` close for its key in the origin's status log, and name on stderr which entry was treated as purged and which record proved it.
The archive is searched under the entry id and, for a concrete origin, under the legacy derived identity too, the same two identities a live row resolves through.
...
The archive a home rotates into is the `.tasks.toml` key when one names it and otherwise tasks-axi's own default beside the backlog file, because a config that names no archive still has one.
