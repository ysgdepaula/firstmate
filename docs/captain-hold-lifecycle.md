...
The archive is searched under the entry id and, for a concrete origin, under the legacy derived identity too, the same two identities a live row resolves through, and the status log is looked up under those same identities because a pre-collapse call can have been closed on the channel under its composed one.
