...
| [`fm-prior-art.sh`](../bin/fm-prior-art.sh) | Find existing evidence in this home's records before commissioning an investigation |
...
| `fm-push-lock.sh`        | Machine-wide publication lock serializing pushes and merges across every home on one machine; wrapper when executed, acquire/release pair when sourced |
...
| `fm-classify-lib.sh`     | Shared wake classification, durable keyed-decision folds and scans, unread status selection, bounded latest-event snapshots, and the PR-delivery done contract guard |
