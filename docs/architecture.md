...
A ship task recorded as a pull-request delivery mode is additionally held to that contract mechanically: the classifier withholds its newest `done:` carrying no pull-request link when it can durably enqueue a contract reminder and record the budget, leaving the task's status-log verdict working.
After a successful steer, two independent bounded paths reach Firstmate if it does not take: the steering inbox's existing re-ring ladder escalates an unacknowledged reminder as a stale wake, and the guard's own per-task reminder budget stops absorbing so the next such line is presented unchanged.
Guard failures leave the line presentable; `bin/fm-classify-lib.sh` owns the guard's timing and failure rules, with the unchanged inbox ladder owned by `bin/fm-task-inbox-lib.sh`.
Every path that decides whether a status line is a finish shares that one predicate, so the classifier, the always-on watcher's stale-terminal test, the away-mode supervisor's signal and stale wake classifications and its wedge aging, both status-log paths of the authoritative current-state reader, inactive reconciliation's secondmate parent-channel ledger and direct deferral and completion gates, and the retirement of a captain's open decisions cannot disagree about whether a task is done.
A withheld line is published nowhere; an idle pane behind it is absorbed with its wedge timer running, and the recovery backstop suppresses only an occurrence the guard is holding so a budget-exhausted one stays recoverable.
...
Two witnesses warrant it - the reminder budget names that exact sentence at its occurrence endpoint, or the log's newest line is a done that satisfies the contract, so the task delivered afterwards and the handoff it replaced is moot.
...
Reminder decisions and budget resets share the per-task lock owned by `bin/fm-classify-lib.sh`, preventing concurrent readers from spending a second reminder on the same occurrence.
