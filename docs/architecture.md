...
Every path that decides whether a status line is a finish shares that one predicate, so the classifier, the always-on watcher's stale-terminal test, the away-mode supervisor's wedge aging, the authoritative current-state reader, and a secondmate's parent-channel ledger cannot disagree about whether a task is done; an idle pane behind a withheld line is absorbed with its wedge timer running rather than muted, and the recovery backstop suppresses only a line the guard is actively holding so a budget-exhausted one stays recoverable.
