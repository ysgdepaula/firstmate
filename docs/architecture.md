...
A pane whose task worktree holds no such write is deferred on a second, independent kind of evidence: a live `no-mistakes` process bound to that same worktree.
The run step is the ordinary way a validation is seen, but `bin/fm-crew-state.sh` can only attribute one through a bounded `no-mistakes axi status` call, so a call that times out or answers for another branch leaves a crew in a long fix or review round looking exactly like a wedged one.
Binding is by working directory, a kernel fact, and the process must also name the validation binary (`FM_NM_PROCESS_NAME`); either signal alone defers nothing, because a leftover process in the worktree and another lane's run elsewhere are both indistinguishable from a wedge, and shared no-mistakes infrastructure (`FM_NM_PROCESS_SHARED_SUBCOMMANDS`) is excluded even when both hold.
Firstmate's own no-mistakes invocations are excluded too, by launcher rather than by subcommand: reading a crew's run step runs a bounded `no-mistakes axi status` inside that crew's own worktree, and a real fix round shares its first argument, so every one of firstmate's own invocations is marked where another process can read it and a marked process is never counted as crew progress.
A candidate whose launcher cannot be established at all is not counted either, on the same rule that governs every other unknown here.
Both deferrals re-surface on the same `FM_PAUSE_RESURFACE_SECS` cadence as a declared wait, with a reason naming the evidence rather than a wedge.
They share ONE deferral chain per window, so the cadence measures how long that window has been deferred rather than how long one source has been answering, and evidence changing hands mid-window cannot restart it.
Each is bounded and taken only in the branch that was about to escalate, never on every poll: one pruned, depth-bounded, wall-clock-bounded walk (`FM_WORKTREE_WRITE_PRUNE`, `FM_WORKTREE_WRITE_MAXDEPTH`, `FM_WORKTREE_WRITE_TIMEOUT`) and one wall-clock-bounded process scan (`FM_NM_PROCESS_TIMEOUT`).
Every absence of evidence, including a missing worktree record, a torn-down worktree, an absent `lsof`, a probe that outlives its wall-clock bound, and a failed probe, leaves the existing escalation schedule untouched, so a crew that produces neither still escalates exactly as before.
A provisioned firstmate home is never probed by either deferral, whether it is recorded by a secondmate or by an ordinary crew, because such a home runs its own supervision and its own validation inside itself: its state directory keeps churning and its own `no-mistakes` runs keep living there whether or not the task that recorded it produces anything, so panes recording one keep escalating on the unchanged schedule.
The process scan itself is system-wide, so the watcher captures it at most once per poll cycle and reads every window's answer from that one capture, and it discards the capture at the top of the next cycle so no escalation is ever deferred on a process list assembled during an earlier one.
A scan that could not run at all is a fact about the cycle rather than about the window that asked first, so it too is remembered for that cycle, as the failure it was and never as an empty process list: the bound is paid once instead of once per window, while every window that asked still escalates exactly as it would have.
A busy pane is otherwise exempt from staleness, but only until its latest `state/<id>.turn-ended` marker reaches `FM_BUSY_TURN_MAX_SECS`, or its `state/<id>.meta` spawn record reaches that age before any turn completes; past that bound it is routed through the same wedge escalation, with the identical reason, escalation count, progress deferrals, and `demand-deep-inspection` marker, for inspection only - never an automatic interrupt, signal, or restart.
...
The pane-churn fallback never reaches a wake naming any status file, and it is unavailable to an entire batch that references a secondmate.
...
A batch whose every task is behind a `done:` withheld by the pull-request delivery contract described below is benign on that hold alone, since its worker already carries the durable steer that event produced.
...
For the same reason the cadence also survives the pane's busy verdict changing under a standing declaration, and every watcher generation: a parked pane reads busy whenever its harness redraws or it holds a foreground call in view, and only a status append that stops declaring the wait ends the declaration, so such a poll resets per-hash bookkeeping alone and leaves the declaration-scoped flag, recheck, and throttle in place.
...
A ship task recorded as a pull-request delivery mode is additionally held to that contract mechanically: a `done:` carrying no pull-request link is not a captain event at all, so instead of presenting it the classifier enqueues one contract reminder into that task's own steering inbox and leaves the task reading as working.
Firstmate is reached only when that steer does not take, along two independent bounded paths - the steering inbox's existing re-ring ladder escalates an unacknowledged reminder as a stale wake, and the guard's own per-task reminder budget stops absorbing so the next such line is presented unchanged - and a worker that cannot be steered at all has its line presented immediately rather than withheld.
Every path that decides whether a status line is a finish shares that one predicate, so the classifier, the always-on watcher's stale-terminal test, the away-mode supervisor's signal and stale wake classifications and its wedge aging, both status-log paths of the authoritative current-state reader, both finish tests of inactive reconciliation (a secondmate's parent-channel ledger, and the direct path's deferral to that ledger), and the retirement of a captain's open decisions cannot disagree about whether a task is done; a withheld line is published nowhere, with one named exception - inactive reconciliation may publish a terminal outcome whose run-step verdict carries a recorded pull request, because green CI on a real pull request is the contract's own proof rather than the worker's prose, and publishing it also supersedes that prose line so its author stops being steered about it, after which a supervisor absorbing a wake on the strength of an active steer no longer counts that task, while the line itself stays unpresentable; an idle pane behind a withheld line is absorbed with its wedge timer running rather than muted, and the recovery backstop suppresses only a line the guard is actively holding so a budget-exhausted one stays recoverable.
The guard judges current state and never history: only a log's newest line is acted on, and a linkless done replayed by a whole-log re-read is dropped rather than re-steered.
That drop is conditional on the line having been judged already, because one classification covers every byte appended since the cursor and a done can therefore be seen for the first time already non-newest.
...
Inbox polls retry interrupted archival independently of later status lines, excluding only cancelled bindings from delivery and escalation.
When cancellations exist, doorbells name only the selected instruction so unrelated delivery cannot direct the worker to a cancelled reminder still awaiting archival.
...
`local-only` and any task with no recorded delivery mode are outside that contract and classify exactly as before.
...
Away-mode housekeeping has neither progress deferral of its own, so while `state/.afk` exists a quiet crew that is writing its own worktree, or running its own validation, still escalates as a possible wedge at that bound.
...
It owns the exact terminal `done:` line each mode requires as one string, and the supervisor's done contract guard quotes that same string back to a worker whose `done:` misses it, so the form a worker is told and the form it is held to cannot drift apart.
