...
Every path that decides whether a status line is a finish shares that one predicate, so the classifier, the always-on watcher's stale-terminal test, the away-mode supervisor's signal and stale wake classifications and its wedge aging, both status-log paths of the authoritative current-state reader, a secondmate's parent-channel ledger, and the retirement of a captain's open decisions cannot disagree about whether a task is done; an idle pane behind a withheld line is absorbed with its wedge timer running rather than muted, and the recovery backstop suppresses only a line the guard is actively holding so a budget-exhausted one stays recoverable.
