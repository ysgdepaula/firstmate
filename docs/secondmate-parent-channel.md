...
A child `done:` withheld by its task's pull-request delivery contract is published by neither the ledger route nor the silent-end scan in `bin/fm-inactive-reconcile.sh` ([`architecture.md`](architecture.md) owns that contract).
The worker must write its own conforming `done:` carrying the pull-request link.
