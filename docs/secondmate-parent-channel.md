...
A child `done:` that its task's pull-request delivery contract withholds is not a terminal outcome on either `bin/fm-inactive-reconcile.sh` route above: the ledger declines to deliver it, and the silent-end scan publishes an outcome for it only when the child's own recorded pull request carries the proof its prose lacks ([`architecture.md`](architecture.md) owns that contract).
