...
It covers persisted preference restoration across every session-start reason and a real restart, proves the [Calm working presentation](calm.md) with Calm on and off through a delayed deterministic provider, asserts no Calm status row, verifies operational messages remain exact ordinary user-role session entries and complete exports, and drives genuine 100 by 44, 160 by 36, and 180 by 44 terminal fixtures.
...
`tests/fm-pi-primary-live-e2e.test.sh` also covers the [Calm working presentation](calm.md) on the credentialed provider path and verifies cleanup when the run settles, before continuing its ordinary watcher lifecycle.
