...
A `done:` the pull-request delivery contract guard is actively holding is skipped, because it was never presented as a finish and its worker holds the guard's steer instead; once that guard's reminder budget is spent the same line becomes a real event again and stays recoverable here.
A line superseded by a published run-step outcome is the one exception: it stays skipped whatever its budget, because that outcome already reached the captain and the guard has deliberately stopped steering its worker.
