...
# Non-PR close and retain transitions preserve a timestamped delivery through
# fm-task-events-lib.sh before changing the row, without closing a retained call.
# Its result title is limited to 240 characters; repeated transitions preserve it.
...
fm_backlog_archive_file() {  # <data-dir>
  ...
  archive=$(fm_tasks_axi_archive_from_toml "$root/.tasks.toml") || archive=''
...
fm_backlog_preserve_delivery() {
  local meta=$1 data=$2 id=$3
  shift 3
  local arg pr_close=0 repo='' title fields shown
  for arg in "$@"; do
    [ "$arg" != --pr ] || pr_close=1
  done
  if [ "$pr_close" = 0 ]; then
    ...
    if [ "${FM_BACKLOG_ROW_STATE%% *}" != "done" ]; then
      shown=$(fm_backlog_row_show "$data" "$id") || return 1
      fields=$(printf '%s\n' "$shown" | jq -Rn '
        def field($name):
          [.[] | select(startswith("  " + $name + ": ")) | ltrimstr("  " + $name + ": ")][0] // ""
          | if startswith("\"") then fromjson else . end
          | if . == "-" then "" else . end;
        [inputs] | {title:(field("title") | gsub("\\s+"; " ") | if length > 240 then .[:239] + "…" else . end),repo:field("repo")}
      ') || return 1
      title=$(printf '%s' "$fields" | jq -r .title) || return 1
      repo=$(printf '%s' "$fields" | jq -r .repo) || return 1
      [ -n "$title" ] || title=$id
      if [ -z "$repo" ] && [ -n "$meta" ] && [ -f "$meta" ]; then
...
fm_backlog_close_transition() {
  ...
  shift 5
  fm_backlog_preserve_delivery "$meta" "$data" "$id" "$@" || return 1
