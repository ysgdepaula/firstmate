...
state/               runtime records and signals; gitignored
  ...
  .<id>.done-guard   PR-delivery reminder budget and occurrence witnesses; bin/fm-classify-lib.sh owns the format, locking, reset, and teardown contract
...
On a `no-mistakes` or `direct-PR` task, a `done:` carrying no pull-request link is not proof of completion; see [`docs/architecture.md`](docs/architecture.md) for automatic steering, bounded escalation, and cases that remain presentable.
...
Use the state that `bin/fm-crew-state.sh` prints; its header owns classification, including CI-monitor and unreachable-daemon exceptions, and parked approval or fix-review states require the worker to follow the active gate help.
...
Load `projets` when the captain requests `/projets`, a per-project page, or the page of everything waiting on him, and after a project-changing event while either page is open, for its invocation and regeneration procedure.
