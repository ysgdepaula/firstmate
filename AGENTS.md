...
state/               runtime records and signals; gitignored
  ...
  .<id>.done-guard   per-task reminder budget for the PR-delivery done contract guard, holding the count spent and the exact `done:` line it last returned to the worker; written only by fm-classify-lib.sh's guard, removed by teardown
