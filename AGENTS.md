...
state/               runtime records and signals; gitignored
  ...
  .<id>.done-guard   per-task reminder budget for the PR-delivery done contract guard, append-only rows with the latest row current: reminders spent<TAB>status-log byte length when that reminder was sent<TAB>the exact `done:` line returned to the worker, where the length is what tells one append apart from a later append of the same text, so a hold is refused once it no longer matches the live log; written only by fm-classify-lib.sh's guard, removed by teardown
  .<id>.done-superseded  per-task markers, append-only rows with the latest row current, recording that a withheld `done:` was answered by a published run-step outcome carrying the task's recorded pull request: status-log byte length when it was published<TAB>that log's identity<TAB>the exact `done:` line it answered, so the guard stops steering the worker about that sentence while a later append of the same text is still judged on its own; the identity field is what stops a marker left by an earlier task of the same reused id from suppressing its successor; written only by fm-inactive-reconcile.sh through fm-classify-lib.sh's guard, removed by teardown
...
On a `no-mistakes` or `direct-PR` task, a `done:` carrying no pull-request link never reads as finished: the worker is returned its exact delivery contract automatically and the task stays working, so such a line reaches you only once that has gone unheeded.
